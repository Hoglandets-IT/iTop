...
class SynchroDataSource extends cmdbAbstractObject
{
	public static function Init()
	{
		...
		MetaModel::Init_AddAttribute(new AttributeEnum('status', [
			'allowed_values' => new ValueSetEnum('implementation,production,obsolete'),
			'sql' => 'status',
			'default_value' => 'implementation',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeExternalKey('user_id', [
			'targetclass' => 'User',
			'jointype' => null,
			'allowed_values' => null,
			'sql' => 'user_id',
		...
		MetaModel::Init_AddAttribute(new AttributeExternalKey('notify_contact_id', [
			'targetclass' => 'Contact',
			'jointype' => null,
			'allowed_values' => null,
			'sql' => 'notify_contact_id',
			'is_null_allowed' => true,
			'on_target_delete' => DEL_MANUAL,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeClass('scope_class', [
		...
		MetaModel::Init_AddAttribute(new AttributeString('database_table_name', [
			'allowed_values' => null,
			'sql' => 'database_table_name',
			'default_value' => null,
			'is_null_allowed' => true,
			'depends_on' => [],
		...
		MetaModel::Init_AddAttribute(new AttributeString('scope_restriction', [
			'allowed_values' => null,
			'sql' => 'scope_restriction',
			'default_value' => null,
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		...
		// Format: seconds (int)
		MetaModel::Init_AddAttribute(new AttributeDuration('full_load_periodicity', [
			'allowed_values' => null,
			'sql' => 'full_load_periodicity',
			'default_value' => 0,
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		...
		MetaModel::Init_AddAttribute(new AttributeEnum('reconciliation_policy', [
			'allowed_values' => new ValueSetEnum('use_primary_key,use_attributes'),
			'sql' => 'reconciliation_policy',
			'default_value' => 'use_attributes',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeEnum('action_on_zero', [
			'allowed_values' => new ValueSetEnum('create,error'),
			'sql' => 'action_on_zero',
			'default_value' => 'create',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeEnum('action_on_one', [
			'allowed_values' => new ValueSetEnum('update,error'),
			'sql' => 'action_on_one',
			'default_value' => 'update',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeEnum('action_on_multiple', [
			'allowed_values' => new ValueSetEnum('take_first,create,error'),
			'sql' => 'action_on_multiple',
			'default_value' => 'error',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));

		MetaModel::Init_AddAttribute(new AttributeEnum('delete_policy', [
			'allowed_values' => new ValueSetEnum('ignore,delete,update,update_then_delete'),
			'sql' => 'delete_policy',
			'default_value' => 'ignore',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeString('delete_policy_update', [
			'allowed_values' => null,
			'sql' => 'delete_policy_update',
			'default_value' => null,
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		...
		MetaModel::Init_AddAttribute(new AttributeDuration('delete_policy_retention', [
			'allowed_values' => null,
			'sql' => 'delete_policy_retention',
			'default_value' => null,
			'is_null_allowed' => true,
			'depends_on' => [],
		]));

		MetaModel::Init_AddAttribute(new AttributeLinkedSet('attribute_list', [
			'linked_class' => 'SynchroAttribute',
			'ext_key_to_me' => 'sync_source_id',
			'allowed_values' => null,
			'count_min' => 0,
			'count_max' => 0,
			'depends_on' => [],
			'tracking_level' => LINKSET_TRACKING_DETAILS,
		]));
		// Not used yet !
		MetaModel::Init_AddAttribute(new AttributeEnum('user_delete_policy', [
			'allowed_values' => new ValueSetEnum('everybody,administrators,nobody'),
			'sql' => 'user_delete_policy',
			'default_value' => 'nobody',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));

		MetaModel::Init_AddAttribute(new AttributeURL('url_icon', [
			'allowed_values' => null,
			'sql' => 'url_icon',
			'default_value' => null,
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeString('url_application', [
			'allowed_values' => null,
			'sql' => 'url_application',
			'default_value' => null,
			'is_null_allowed' => true,
		...
		MetaModel::Init_SetZListItems(
			...
			[
				'col:0' => [
					'fieldset:SynchroDataSource:Description' => [
						'name',
						'description',
						'status',
						'scope_class',
				...
				'col:1' => [
					'fieldset:SynchroDataSource:Reconciliation' => [
						'reconciliation_policy',
						'action_on_zero',
						'action_on_one',
						'action_on_multiple',
					],
					'fieldset:SynchroDataSource:Deletion' => [
						'user_delete_policy',
						'full_load_periodicity',
						'delete_policy',
		...
		MetaModel::Init_SetZListItems(
			'list',
			['scope_class', 'status', 'user_id', 'full_load_periodicity']
		); // Attributes to be displayed for a list
		// Search criteria
		MetaModel::Init_SetZListItems(
			'standard_search',
			['name', 'status', 'scope_class', 'user_id']
		); // Criteria of the std search form
		MetaModel::Init_SetZListItems('default_search', ['name', 'status', 'scope_class']); // Criteria of the defaut search form
	...
	public function DisplayBareRelations(WebPage $oPage, $bEditMode = false)
	{
		if (!$this->IsNew()) {
			...
			// Columns of the form
			$aAttribs = [];
			foreach (['attcode', 'reconciliation', 'update', 'update_policy', 'reconciliation_attcode'] as $s) {
				$aAttribs[$s] = ['label' => Dict::S("Core:SynchroAtt:$s"), 'description' => Dict::S("Core:SynchroAtt:$s+")];
			}
			// Rows of the form
			$aValues = [];
				...
				if ($oAttDef->IsWritable()) {
					...
					if ($oAttribute !== null) {
						if (!$bEditMode) {
							...
							$aRow['attcode'] = MetaModel::GetLabel(
								$this->GetTargetClass(),
								$oAttribute->Get('attcode')
							).' ('.$oAttribute->Get('attcode').')';
							...
							if ($oAttDef->IsExternalKey()) {
								...
								switch ($sReconciliationAttCode) {
									...
									default:
										$sDisplayReconciliationAttCode = MetaModel::GetLabel(
											$oAttDef->GetTargetClass(),
											$sReconciliationAttCode
										);
								}
							...
							$aRow['update_policy'] = cmdbAbstractObject::GetFormElementForField(
								$oPage,
								get_class($oAttribute),
								'update_policy',
								$oUpdateAttDef,
								$oAttribute->Get('update_policy'),
								'',
								'update_policy_'.$sAttCode,
								"[$sAttCode]"
							);
							if ($oAttDef->IsExternalKey()) {
								$aRow['reconciliation_attcode'] = $oAttribute->GetReconciliationFormElement(
									$oAttDef->GetTargetClass(),
									"attr_reconciliation_attcode[$sAttCode]"
								);
							} else {
								$aRow['reconciliation_attcode'] = '&nbsp;';
							}
	...
	protected function DisplayStatusTab(WebPage $oPage)
	{
		...
		$oSetSynchroLog = new CMDBObjectSet(
			DBObjectSearch::FromOQL($sSelectSynchroLog),
			['start_date' => false] /* order by*/,
...
JS;
			...
			$sNbReplica = '<img src="'.$this->GetIcon(false).'" class="ibo-class-icon ibo-is-medium" style="vertical-align:middle;">&nbsp;'.Dict::Format(
				'Core:Synchro:Nb_Replica',
				"<span id=\"c_nb_replica_total\">{$aData['nb_replica_total']}</span>"
			);
			$sNbObjects = '<img src="'.MetaModel::GetClassIcon($this->GetTargetClass(), false).'" class="ibo-class-icon ibo-is-medium" style="vertical-align:middle;">&nbsp;'.Dict::Format(
				'Core:Synchro:Nb_Class:Objects',
				$this->GetTargetClass(),
				"<span id=\"c_nb_obj_total\">{$aData['nb_obj_total']}</span>"
			);
			$oPage->add(
				<<<EOF
...
EOF
			...
			).'<td rowspan="4" class="arrow"><i class="fas fa-arrow-right"></i></td>'.$this->HtmlBox('obj_disappeared_no_action', $aData, 'grey', '', '', 'ibo-data-synchro-source--replicas-status-separator'));
			$oPage->add("</tr>\n<tr>");
			...
			$oPage->add($this->HtmlBox(
				...
				'',
				'ibo-data-synchro-source--replicas-status-separator'
			).'<td rowspan="3" class="arrow"><i class="fas fa-arrow-right"></i></td>'.$this->HtmlBox('obj_unchanged', $aData, 'blue', '', '', 'ibo-data-synchro-source--replicas-status-separator'));
			$oPage->add("</tr>\n<tr>");
			...
			$oPage->add($this->HtmlBox(
				...
				'',
				'ibo-data-synchro-source--replicas-status-separator'
			).'<td rowspan="4" class="arrow"><i class="fas fa-arrow-right"></i></td>'.$this->HtmlBox('obj_new_unchanged', $aData, 'blue', '', '', 'ibo-data-synchro-source--replicas-status-separator'));
			$oPage->add("</tr>\n<tr>");
	...
	public function UpdateObjectFromPostedForm($sFormPrefix = '', $sAttList = null, $aAttFlags = [])
	{
		...
		$aReconcile = utils::ReadPostedParam('reconciliation', []);
		$aUpdate = utils::ReadPostedParam('update', []);
		$aUpdatePolicy = utils::ReadPostedParam('attr_update_policy', []);
		$aReconciliation = utils::ReadPostedParam('attr_reconciliation_attcode', []);
			...
			if (!isset($aAttributes[$sAttCode])) {
				...
				foreach ($aColumns as $sColName => $sColumnDef) {
					...
					if (!CMDBSource::IsField($sTable, $sColName)) {
						...
						$bOneColIsMissing = true;
						if (count($aColumns) > 1) {
							echo "Incorrect column '$sColName' (".CMDBSource::GetFieldSpec(
								$sTable,
								$sColName
							).' instead of '.$sColumnDef."), in the table '$sTable' for the data synchro task ".$this->GetName().' ('.$this->GetKey()."). The columns '".implode(
								"', '",
								$aColumns
							)." will be re-created.'.\n";
						} else {
							echo "Incorrect column '$sColName' (".CMDBSource::GetFieldSpec(
								$sTable,
								$sColName
							).' instead of '.$sColumnDef."), in the table '$sTable' for the data synchro task ".$this->GetName().' ('.$this->GetKey()."). The column '$sColName' will be re-created.\n";
						}
					}
					if ($bOneColIsMissing) {
						$bTriggerRebuildNeeded = true;
...
class SynchroAttribute extends cmdbAbstractObject
{
	public static function Init()
	{
		...
		MetaModel::Init_Params($aParams);
		MetaModel::Init_InheritAttributes();
		MetaModel::Init_AddAttribute(new AttributeExternalKey('sync_source_id', [
			'targetclass' => 'SynchroDataSource',
			'jointype' => '',
			'allowed_values' => null,
			'sql' => 'sync_source_id',
			'is_null_allowed' => false,
		...
		MetaModel::Init_AddAttribute(new AttributeExternalField(
			'sync_source_name',
			['allowed_values' => null, 'extkey_attcode' => 'sync_source_id', 'target_attcode' => 'name']
		));
		MetaModel::Init_AddAttribute(new AttributeString('attcode', [
			'allowed_values' => null,
			'sql' => 'attcode',
			'default_value' => null,
			'is_null_allowed' => false,
		...
		MetaModel::Init_AddAttribute(new AttributeBoolean('update', [
			'allowed_values' => null,
			'sql' => 'update',
			'default_value' => true,
			'is_null_allowed' => false,
		...
		MetaModel::Init_AddAttribute(new AttributeBoolean('reconcile', [
			'allowed_values' => null,
			'sql' => 'reconcile',
			'default_value' => false,
			'is_null_allowed' => false,
		...
		MetaModel::Init_AddAttribute(new AttributeEnum('update_policy', [
		...
		MetaModel::Init_SetZListItems(
			'list',
			['sync_source_id', 'attcode', 'update', 'reconcile', 'update_policy']
		); // Attributes to be displayed for a list
		// Search criteria
...
class SynchroAttExtKey extends SynchroAttribute
{
	public static function Init()
	{
		...
		MetaModel::Init_Params($aParams);
		MetaModel::Init_InheritAttributes();
		MetaModel::Init_AddAttribute(new AttributeString('reconciliation_attcode', [
			'allowed_values' => null,
			'sql' => 'reconciliation_attcode',
			'default_value' => null,
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
	...
	public function GetReconciliationFormElement($sTargetClass, $sFieldName)
	{
		...
		foreach (MetaModel::ListAttributeDefs($sTargetClass) as $sAttCode => $oAttDef) {
			if ($oAttDef->IsScalar() && ($sAttCode != 'friendlyname')) {
				...
				$aMoreOptions[MetaModel::GetLabel(
					$sTargetClass,
					$sAttCode
				)] = "<option value=\"$sAttCode\" $sSelected>".MetaModel::GetLabel($sTargetClass, $sAttCode)."</option>\n";
			}
		}
		ksort($aMoreOptions);
...
class SynchroAttLinkSet extends SynchroAttribute
{
	public static function Init()
	{
		...
		MetaModel::Init_Params($aParams);
		MetaModel::Init_InheritAttributes();
		MetaModel::Init_AddAttribute(new AttributeString('row_separator', [
			'allowed_values' => null,
			'sql' => 'row_separator',
			'default_value' => '|',
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeString('attribute_separator', [
			'allowed_values' => null,
			'sql' => 'attribute_separator',
			'default_value' => ';',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeString('value_separator', [
			'allowed_values' => null,
			'sql' => 'value_separator',
			'default_value' => ':',
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeString('attribute_qualifier', [
			'allowed_values' => null,
...
class SynchroLog extends DBObject
{
	public static function Init()
	{
		...
		MetaModel::Init_AddAttribute(new AttributeDateTime('start_date', [
			'allowed_values' => null,
			'sql' => 'start_date',
			'default_value' => '',
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeDateTime('end_date', [
			'allowed_values' => null,
			'sql' => 'end_date',
			'default_value' => '',
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeEnum('status', [
			'allowed_values' => new ValueSetEnum('running,completed,error'),
			'sql' => 'status',
			'default_value' => 'running',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('status_curr_job', [
			'allowed_values' => null,
			'sql' => 'status_curr_job',
			'default_value' => 0,
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeInteger('status_curr_pos', [
			'allowed_values' => null,
			'sql' => 'status_curr_pos',
			'default_value' => 0,
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_replica_seen', [
			'allowed_values' => null,
			'sql' => 'stats_nb_replica_seen',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_replica_total', [
			'allowed_values' => null,
			'sql' => 'stats_nb_replica_total',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_deleted', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_deleted',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_deleted_errors', [
			'allowed_values' => null,
			'sql' => 'stats_deleted_errors',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_obsoleted', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_obsoleted',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_obsoleted_errors', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_obsoleted_errors',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_created', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_created',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_created_errors', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_created_errors',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_created_warnings', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_created_warnings',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_updated', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_updated',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_updated_errors', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_updated_errors',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_updated_warnings', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_updated_warnings',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_unchanged_warnings', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_unchanged_warnings',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		...
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_replica_reconciled_errors', [
			'allowed_values' => null,
			'sql' => 'stats_nb_replica_reconciled_errors',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_replica_disappeared_no_action', [
			'allowed_values' => null,
			'sql' => 'stats_nb_replica_disappeared_no_action',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));

		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_new_updated', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_new_updated',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_new_updated_warnings', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_new_updated_warnings',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_new_unchanged', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_new_unchanged',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeInteger('stats_nb_obj_new_unchanged_warnings', [
			'allowed_values' => null,
			'sql' => 'stats_nb_obj_new_unchanged_warnings',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));

		MetaModel::Init_AddAttribute(new AttributeText('last_error', [
			'allowed_values' => null,
			'sql' => 'last_error',
			'default_value' => '',
			'is_null_allowed' => true,
		...
		MetaModel::Init_AddAttribute(new AttributeLongText(
			'traces',
			['allowed_values' => null, 'sql' => 'traces', 'default_value' => '', 'is_null_allowed' => true, 'depends_on' => []]
		));

		MetaModel::Init_AddAttribute(new AttributeInteger('memory_usage_peak', [
			'allowed_values' => null,
			'sql' => 'memory_usage_peak',
			'default_value' => 0,
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		...
		MetaModel::Init_SetZListItems('details', [
			...
			'stats_nb_replica_seen',
			'stats_nb_obj_created', /*'stats_nb_replica_reconciled',*/ 'stats_nb_obj_updated',
			'stats_nb_obj_obsoleted',
			'stats_nb_obj_deleted',
			'stats_nb_obj_created_errors',
			'stats_nb_replica_reconciled_errors',
			'stats_nb_replica_disappeared_no_action',
			'stats_nb_obj_updated_errors',
			'stats_nb_obj_obsoleted_errors',
			'stats_nb_obj_deleted_errors',
			'stats_nb_obj_new_unchanged',
			'stats_nb_obj_new_updated',
			'traces',
		]); // Attributes to be displayed for the complete details
		MetaModel::Init_SetZListItems(
			'list',
			['sync_source_id', 'start_date', 'end_date', 'status', 'stats_nb_replica_seen']
		); // Attributes to be displayed for a list
		// Search criteria
		//		MetaModel::Init_SetZListItems('standard_search', array('name')); // Criteria of the std search form
		//		MetaModel::Init_SetZListItems('advanced_search', array('name')); // Criteria of the advanced search form
...
{
	public static $aSearches = []; // Cache of OQL queries used for reconciliation (per data source)
	protected $aWarnings;
	...
	public static function Init()
	{
		...
		MetaModel::Init_AddAttribute(new AttributeExternalField(
			'base_class',
			['allowed_values' => null, 'extkey_attcode' => 'sync_source_id', 'target_attcode' => 'scope_class']
		));

		MetaModel::Init_AddAttribute(new AttributeObjectKey('dest_id', [
			'allowed_values' => null,
			'class_attcode' => 'dest_class',
			'sql' => 'dest_id',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeClass('dest_class', [
			'class_category'       => '',
			'more_values'          => '',
			'sql'                  => 'dest_class',
			'default_value' => '',
		...
		MetaModel::Init_AddAttribute(new AttributeDateTime('status_last_seen', [
			'allowed_values' => null,
		...
		MetaModel::Init_AddAttribute(new AttributeEnum('status', [
			'allowed_values' => new ValueSetEnum('new,synchronized,modified,orphan,obsolete'),
			'sql' => 'status',
			'default_value' => 'new',
			'is_null_allowed' => false,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeBoolean('status_dest_creator', [
			'allowed_values' => null,
			'sql' => 'status_dest_creator',
			'default_value' => 0,
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeString('status_last_error', [
			'allowed_values' => null,
			'sql' => 'status_last_error',
			'default_value' => '',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeString('status_last_warning', [
			'allowed_values' => null,
			'sql' => 'status_last_warning',
			'default_value' => '',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));

		MetaModel::Init_AddAttribute(new AttributeDateTime('info_creation_date', [
			'allowed_values' => null,
			'sql' => 'info_creation_date',
			'default_value' => '',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		MetaModel::Init_AddAttribute(new AttributeDateTime('info_last_modified', [
			'allowed_values' => null,
			'sql' => 'info_last_modified',
			'default_value' => '',
			'is_null_allowed' => true,
			'depends_on' => [],
		]));
		...
		MetaModel::Init_SetZListItems(
			...
			[
				''.
				'col:0' => [
					'fieldset:SynchroDataSource:Definition' => ['sync_source_id', 'dest_id', 'dest_class'],
					'fieldset:SynchroDataSource:Status' => [
						'status',
						'status_last_seen',
						'status_dest_creator',
						'status_last_error',
					...
					'fieldset:SynchroDataSource:Information' => ['info_creation_date', 'info_last_modified'],
				],
			]
		);
			...
			case 'new':
				...
				foreach ($aReconciliationKeys as $sFilterCode => $oSyncAtt) {
					...
					if (!is_null($value)) {
						...
						$oStatLog->AddTrace(
							"<<< End of SyncroReplica::Synchro (error could not reconcile on null value for attribute '$sFilterCode').",
							$this
						);
				...
				$oDestSet = new DBObjectSet(self::$aSearches[$oDataSource->GetKey()], [], $aFilterValues);
				...
				switch ($iCount) {
					...
					case 1:
						...
						if ($oDataSource->Get('action_on_one') == 'update') {
							...
							$oStatLog->AddTrace(
								'Calling UpdateObjectFromReplica('.(get_class($oDestObj).'::'.$oDestObj->GetKey()).')',
								$this
							);
							$bModified = $this->UpdateObjectFromReplica(
								$oDestObj,
								$aAttributes,
								$oChange,
								$oStatLog,
	...
	protected function GetValueFromExtData($sExtAttCode, $oSyncAtt, &$oStatLog)
	{
		...
		if (!is_null($oSyncAtt) && ($oSyncAtt instanceof SynchroAttExtKey)) {
			...
			if (!empty($sReconcAttCode)) {
				...
				if ($oObj) {
					...
					if ($rawValue != '') {
						...
						$oStatLog->AddTrace(
							"Could not find [unique] object for '$sExtAttCode': searched on $sReconcAttCode = '$rawValue'",
							$this
						);
			...
			$retValue = $oAttDef->MakeValueFromString(
				$rawValue,
				$bLocalizedValue,
				$oSyncAtt->Get('row_separator'),
				$oSyncAtt->Get('attribute_separator'),
				$oSyncAtt->Get('value_separator'),
				$oSyncAtt->Get('attribute_qualifier')
			);
		} else {
	...
	public function DisplayBareProperties(WebPage $oPage, $bEditMode = false, $sPrefix = '', $aExtraParams = [])
	{
		...
		foreach ($aZList as $sAttCode) {
			...
			$aDetails[] = [
				'label' => '<span title="'.MetaModel::GetDescription($sClass, $sAttCode).'">'.MetaModel::GetLabel(
					$sClass,
					$sAttCode
				).'</span>',
				'value' => $sDisplayValue,
		...
		if ($bCanDisplayDestObjSections) {
			...
			$aHeaders = [
				'attcode' => ['label' => Dict::S('UI:Form:Property'), 'description' => ''],
				'data'    => ['label' => Dict::S('UI:Form:Value'), 'description' => ''],
			];
			$aRows = [];
...
class SynchroExecution
{
	...
	protected $m_aExtDataSpec = [];
	protected $m_aReconciliationKeys = [];
	 ...
	 * @param DateTime $oImportPhaseStartDate Start of the import when doing both import and exec phases in the same script. (This is used
	 *     by `synchro_import.php --synchronize=1`) null when exec phase is used alone and the import start date is unknown.
	 *
	 * @throws \CoreException
	 */
	...
	protected function PrepareLogs()
	{
		...
		$oSetTotal = new DBObjectSet(
			DBObjectSearch::FromOQL($sSelectTotal),
			[] /* order by*/,
	...
	public function PrepareProcessing($bFirstPass = true)
	{
		...
		$oSetAtt = new DBObjectSet(
			DBObjectSearch::FromOQL($sSelectAtt),
			[] /* order by*/,
			['source_id' => $this->m_oDataSource->GetKey()] /* aArgs */
		);
		...
		if ($this->m_oDataSource->Get('reconciliation_policy') == 'use_attributes') {
			...
			$this->m_aReconciliationKeys = ['primary_key' => null];
		}
	...
	public function Process()
	{
		...
		try {
			...
			if ($iErrors > 0) {
				...
				$this->m_oDataSource->SendNotification(
					"errors ($iErrors)",
					"<p>The synchronization has been executed, $iErrors errors have been encountered. Click <a href=\"$sIssuesURL\">here</a> to see the records being currently in error.</p>".$sStatistics
				);
			} else {
	...
	protected function DoJob1UpdateReplicas($iMaxReplica = null, $iCurrPos = -1)
	{
		...
		if ($sDeletePolicy !== 'ignore') {
			...
			$oSetScope = new DBObjectSet(DBObjectSearch::FromOQL($sSelectToObsolete), [] /* order by*/, [
				'source_id' => $this->m_oDataSource->GetKey(),
			...
			if ($iMaxReplica) {
				...
				$oSetToProcess = new DBObjectSet(DBObjectSearch::FromOQL($sSelectToObsolete), ['id' => true] /* order by*/, [
					'source_id' => $this->m_oDataSource->GetKey(),
					'last_import' => $sLastFullLoadStartDate,
					'curr_pos' => $iCurrPos,
				]);
			...
			while ($oReplica = $oSetToProcess->Fetch()) {
				...
				switch ($sDeletePolicy) {
					...
					case 'update_then_delete':
						...
						$aToUpdate = [];
						$aToUpdateSpec = explode(
							';',
							$this->m_oDataSource->Get('delete_policy_update')
		...
		$oSetSeen = new DBObjectSet(
			DBObjectSearch::FromOQL($sSelectSeen),
			[] /* order by*/,
			['source_id' => $this->m_oDataSource->GetKey(), 'last_import' => $sLastFullLoadStartDate]
		);
	...
	protected function DoJob2CreateOrUpdateItopObjects($iMaxReplica = null, $iCurrPos = -1)
	{
		...
		$oSetScope = new DBObjectSet(DBObjectSearch::FromOQL($sSelectToSync), [], [
			'source_id' => $this->m_oDataSource->GetKey(),
		...
		if ($iMaxReplica) {
			// Consider a given subset, starting from replica iCurrPos, limited to the count of iMaxReplica
			// The replica have to be ordered by id
			$oSetToProcess = new DBObjectSet(
				DBObjectSearch::FromOQL($sSelectToSync),
				['id' => true] /* order by*/,
				['source_id' => $this->m_oDataSource->GetKey(), 'last_import' => $sLimitDate, 'curr_pos' => $iCurrPos],
				$this->m_aExtDataSpec
			);
			$oSetToProcess->SetLimit($iMaxReplica);
		} else {
	...
	protected function DoJob3DeleteItopObjectsAndReplicas($iMaxReplica = null, $iCurrPos = -1)
	{
		...
		$oSetScope = new DBObjectSet(DBObjectSearch::FromOQL($sSelectToDelete), [], [
			'source_id' => $this->m_oDataSource->GetKey(),
		...
		if ($iMaxReplica) {
			...
			// The replica have to be ordered by id
			$oSetToProcess = new DBObjectSet(
				DBObjectSearch::FromOQL($sSelectToDelete),
				['id' => true] /* order by*/,
				['source_id' => $this->m_oDataSource->GetKey(), 'last_import' => $sDeletionDate, 'curr_pos' => $iCurrPos]
			);
			$oSetToProcess->SetLimit($iMaxReplica);
		} else {
			$oSetToProcess = $oSetScope;
